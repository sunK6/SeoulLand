...
*/ ///////////////////////////////////////////
function showGNB(gubun) { //gubun-오버/아웃시 구분값 전달변수(1-오버시/0-아웃시)
    // 1.함수호출확인
    // console.log("GNB배경보여" + gubun);

    // 2.변경대상:.gnbbx(gnb배경박스)
    var tg = document.getElementsByClassName("gnbbx").item(0);
    // 클래스를 불러올땐 반드시 순번 item(순번)을 써야함!

    // 3.변경내용: 대상에 클래스 "on"을 오버시엔 넣고 아웃시엔 뺀다!
    if (gubun) tg.classList.add("on");
    // if(gubun===1) 1값은 true이므로 안써도됨!
    // if문은 실행문이 하나일경우 중괄호{} 생략가능!
    else tg.classList.remove("on");
    // else는 if조건이 아닌경우임. 지금은 gubun값이 0인경우 

    // if문 없이 클래스 "on"이 있으면 넣고 없으면 빼는 toggle매서드 사용할 수도 있음!
...
$(function () { /////// jQB //////////////////
    ...
    //// 패밀리사이트 연결하기 ///////
    // 대상: #fslink
    // 이벤트: select박스의 선택 option이 변경될때 -> change
    ...
    //            + 모바일 전체 메뉴 fadeIn/fadeOut -> fadToggle()
    $("#ham").click(function(){
        // 1. 클릭된 햄버거 버튼에 클래스 넣기/빼기
        $(this).toggleClass("on");
